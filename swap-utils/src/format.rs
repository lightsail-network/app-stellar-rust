...
pub fn format_stellar_address(public_key: &[u8; 32], buffer: &mut [u8]) -> Result<usize, ()> {
    if buffer.len() < 56 {
        return Err(());
    }
...
pub fn format_printable_amount<'a>(amount: &[u8; 16], buffer: &'a mut [u8]) -> Result<&'a str, ()> {
    ...
    core::str::from_utf8(&buffer[..written_len]).map_err(|_| ())
}
...
fn encode_base32(input: &[u8], output: &mut [u8]) -> Result<(), ()> {
    if input.len() != 35 {
        return Err(());
    }
...
fn format_xlm_amount(num: u64, writer: &mut BufferWriter) -> Result<(), ()> {
...
fn write_integer_u64(mut value: u64, writer: &mut BufferWriter) -> Result<(), ()> {
    if value == 0 {
        writer.write_byte(b'0')?;
        return Ok(());
    }
...
impl<'a> BufferWriter<'a> {
    ...
    fn write_bytes(&mut self, bytes: &[u8]) -> Result<(), ()> {
        ...
        let new_pos = self.pos.checked_add(bytes.len()).ok_or(())?;
...
mod tests {
    ...
    fn test_format_stellar_address_buffer_validation() {
        ...
        assert_eq!(format_stellar_address(&public_key, &mut buffer), Err(()));
    ...
    fn test_buffer_writer() {
        ...
        // Test buffer overflow
        {
            ...
            assert_eq!(writer.write_bytes(b"12345"), Err(()));
    ...
    fn test_encode_base32() {
        ...
        assert_eq!(encode_base32(&input, &mut small_output), Err(()));
    }
    ...
    fn test_write_decimal_digits_u64() {
        ...
        let mut writer = BufferWriter::new(&mut buffer);
        assert_eq!(write_decimal_digits_u64(123, 8, &mut writer), Err(()));
    }
}
