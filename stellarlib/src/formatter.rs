...
fn format_allow_trust_op(op: &AllowTrustOp) -> Vec<DataEntry> {
    let mut entries = Vec::new();
    entries.push(DataEntry::new("Operation Type", "Allow Trust".to_string()));
    entries.push(DataEntry::new("Trustor", op.trustor.to_string()));
    entries.push(DataEntry::new("Asset Code", op.asset.to_string()));
    entries.push(DataEntry::new(
        "Authorize",
        format_allow_trust_flags(op.authorize),
    ));
    entries
}
...
fn format_account_merge_op(destination: &MuxedAccount) -> Vec<DataEntry> {
    ...
    entries.push(DataEntry::new("Destination", destination.to_string()));
    entries
}
...
fn format_bump_sequence_op(op: &BumpSequenceOp) -> Vec<DataEntry> {
    ...
    entries.push(DataEntry::new(
        "Operation Type",
        "Bump Sequence".to_string(),
    ));
    entries.push(DataEntry::new("Bump To", op.bump_to.to_string()));
    entries
}
...
fn format_create_claimable_balance_op(op: &CreateClaimableBalanceOp) -> Vec<DataEntry> {
    ...
    for (index, claimant) in op.claimants.iter().enumerate() {
        entries.push(DataEntry::new(
            &format!("Claimant {} of {}", index + 1, claimant_count),
...
fn format_claim_claimable_balance_op(op: &ClaimClaimableBalanceOp) -> Vec<DataEntry> {
    ...
    entries.push(DataEntry::new(
        "Operation Type",
        "Claim Claimable Balance".to_string(),
    ));
    entries.push(DataEntry::new("Balance ID", op.balance_id.to_string()));
    entries
}
...
fn format_begin_sponsoring_future_reserves_op(
    ...
    entries.push(DataEntry::new("Sponsored ID", op.sponsored_id.to_string()));
    entries
}
...
fn format_clawback_op(op: &ClawbackOp) -> Vec<DataEntry> {
    let mut entries = Vec::new();
    entries.push(DataEntry::new("Operation Type", "Clawback".to_string()));
    entries.push(DataEntry::new(
        "Clawback",
        format_amount_with_asset(op.amount, &op.asset),
    ));
    entries.push(DataEntry::new("From", op.from.to_string()));
    entries
}
...
fn format_clawback_claimable_balance_op(op: &ClawbackClaimableBalanceOp) -> Vec<DataEntry> {
    ...
    entries.push(DataEntry::new(
        "Operation Type",
        "Clawback Claimable Balance".to_string(),
    ));
    entries.push(DataEntry::new("Balance ID", op.balance_id.to_string()));
    entries
}
...
fn format_set_trust_line_flags_op(op: &SetTrustLineFlagsOp) -> Vec<DataEntry> {
    ...
    entries.push(DataEntry::new(
        "Operation Type",
        "Set Trust Line Flags".to_string(),
    ));
    entries.push(DataEntry::new("Trustor", op.trustor.to_string()));
    entries.push(DataEntry::new("Asset", format_asset(&op.asset).to_string()));
    entries.push(DataEntry::new(
        "Clear Flags",
        format_trust_line_flags(op.clear_flags),
    ));
    entries.push(DataEntry::new(
        "Set Flags",
        format_trust_line_flags(op.set_flags),
    ));
    entries
}
...
fn format_liquidity_pool_deposit_op(
    ...
    entries.push(DataEntry::new(
        "Operation Type",
        "Liquidity Pool Deposit".to_string(),
    ));
    entries.push(DataEntry::new(
        "Liquidity Pool ID",
        format_pool_id(&op.liquidity_pool_id),
    ));
    entries.push(DataEntry::new(
        "Max Amount A",
    ...
    entries.push(DataEntry::new("Min Price", format_price(&op.min_price)?));
    entries.push(DataEntry::new("Max Price", format_price(&op.max_price)?));
    Ok(entries)
}
...
fn format_liquidity_pool_withdraw_op(op: &LiquidityPoolWithdrawOp) -> Vec<DataEntry> {
    ...
    entries.push(DataEntry::new(
        "Operation Type",
        "Liquidity Pool Withdraw".to_string(),
    ));
    entries.push(DataEntry::new(
        "Liquidity Pool ID",
        format_pool_id(&op.liquidity_pool_id),
    ...
    entries.push(DataEntry::new(
        "Amount",
        format_native_amount(op.amount).to_string(),
    ));
    entries.push(DataEntry::new(
        "Min Amount A",
