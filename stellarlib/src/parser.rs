    ...
    fn parse(parser: &mut Parser<'a>) -> Result<Self, ParseError> {
        ...
        match val_type {
            ...
            ScValType::ScvVec => {
                ...
                let vec_data = parser.parse_optional(|p| VecM::parse(p))?;
                Ok(ScVal::Vec(vec_data))
            }
            ScValType::ScvMap => {
                // Parse optional pointer: if present, parse array of SCMapEntry
                let map_data = parser.parse_optional(|p| VecM::parse(p))?;
            ...
            ScValType::ScvContractInstance => {
                ...
                let storage = parser.parse_optional(|p| VecM::parse(p))?;
...
impl<'a> XdrParse<'a> for ManageDataOp<'a> {
    fn parse(parser: &mut Parser<'a>) -> Result<Self, ParseError> {
        ...
        let data_value = parser.parse_optional(|p| BytesM::parse(p))?;
        Ok(ManageDataOp {
            data_name,
            data_value,
        })
