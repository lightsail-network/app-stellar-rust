...
mod operation_validation_tests {
    // Test that the correct operations are identified and validated

    #[test]
    fn test_transfer_operation_requirements() {
        // Transfer requires exactly 3 arguments:
        // 1. from: Address
        // 2. to: Address
        // 3. amount: i128

        // This is validated by the validate_args! macro
        // and type checking in the formatter
        assert_eq!(3, 3); // Placeholder to show the requirement
    }

    #[test]
    fn test_approve_operation_requirements() {
        // Approve requires exactly 4 arguments:
        // 1. from: Address
        // 2. spender: Address
        // 3. amount: i128
        // 4. expiration_ledger: u32

        assert_eq!(4, 4); // Placeholder to show the requirement
    }

    #[test]
    fn test_transfer_from_operation_requirements() {
        // Transfer_from requires exactly 4 arguments:
        // 1. spender: Address
        // 2. from: Address
        // 3. to: Address
        // 4. amount: i128
    ...
    #[test]
    fn test_mint_operation_requirements() {
        // Mint requires exactly 2 arguments:
        // 1. to: Address
        // 2. amount: i128

        assert_eq!(2, 2); // Placeholder to show the requirement
    }

    #[test]
    fn test_burn_operation_requirements() {
        // Burn requires exactly 2 arguments:
        // 1. from: Address
        // 2. amount: i128
    ...
    #[test]
    fn test_clawback_operation_requirements() {
        // Clawback requires exactly 2 arguments:
        // 1. from: Address
        // 2. amount: i128
    ...
    #[test]
    fn test_burn_from_operation_requirements() {
        // Burn_from requires exactly 3 arguments:
        // 1. spender: Address
        // 2. from: Address
        // 3. amount: i128
...
mod builder_pattern_tests {
    // Test the builder pattern logic used in the formatter

    use stellarlib::formatter::DataEntry;

    #[test]
    fn test_data_entry_creation() {
        let entry = DataEntry::new("Transfer", "10 USDC@Contract".to_string());
        assert_eq!(entry.title, "Transfer");
        assert_eq!(entry.content, "10 USDC@Contract");

        let entry = DataEntry::new("From", "GA123...".to_string());
        assert_eq!(entry.title, "From");
        assert_eq!(entry.content, "GA123...");
    }

    #[test]
    fn test_entry_ordering() {
        // Verify that entries are created in the expected order
        let mut entries = vec![];

        // For a transfer operation
        entries.push(DataEntry::new("Transfer", "10 USDC".to_string()));
        entries.push(DataEntry::new("From", "Address1".to_string()));
        entries.push(DataEntry::new("To", "Address2".to_string()));

        assert_eq!(entries[0].title, "Transfer");
        assert_eq!(entries[1].title, "From");
        assert_eq!(entries[2].title, "To");
    }
}
