...
fn test_sign_tx_format_case(case_name: &str) {
    ...
    let raw_data = fs::read(&raw_path).expect(&format!("Failed to read {}", raw_path));
    let expected_output =
        fs::read_to_string(&txt_path).expect(&format!("Failed to read {}", txt_path));

    let mut parser = Parser::new(&raw_data);
    let tx_signature_payload = TransactionSignaturePayload::parse(&mut parser)
        .expect(&format!("Failed to parse XDR for {}", case_name));
    ...
    let mut entries = format_transaction_signature_payload(&tx_signature_payload, &config, signer)
        .expect(&format!("Failed to format transaction for {}", case_name));
    ...
    for i in 0..op_count {
        ...
        let operation = stellarlib::Operation::parse(&mut parser)
            .expect(&format!("Failed to parse Operation for {}", case_name));
        let op_entries = stellarlib::formatter::format_operation(&operation, &config)
            .expect(&format!("Failed to format Operation for {}", case_name));
...
fn test_soroban_auth_format_case(case_name: &str) {
    ...
    let hash_id_preimage = HashIDPreimage::parse(&mut parser)
        .expect(&format!("Failed to parse XDR for {}", case_name));
    let auth = match hash_id_preimage {
        HashIDPreimage::SorobanAuthorization(auth) => auth,
    };
